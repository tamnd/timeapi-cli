...
describes what users get. Pick a format with `-o`, or let timeapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
timeapi <command> -o table   # aligned columns for reading
timeapi <command> -o jsonl   # one JSON object per line, for piping
timeapi <command> -o json    # a single JSON array
timeapi <command> -o csv     # spreadsheet friendly
timeapi <command> -o tsv     # tab-separated
timeapi <command> -o url     # just the URL column
timeapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
timeapi <command> --fields id,title,url
```
...
```bash
timeapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
timeapi <command>            # a table, because this is a terminal
timeapi <command> | wc -l    # JSONL, because this is a pipe
```
